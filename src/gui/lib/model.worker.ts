...
class WorkerModelEngine {
  ...
  async unloadDirector() {
    if (this.director) {
      if (this.isLowMemory) {
        console.log("🧹 (Worker) Unloading Director Engine in safe/low-memory mode...");
        if (this.director.model) {
          await this.safeDispose(this.director.model);
        }
        await this.safeDispose(this.director);
        this.director = null;
      } else {
        console.log("🌸 (Worker) Keeping Director Engine warm in persistent router cache.");
      }
    }
  }
  ...
  async loadModel(category: string, modelId: string, sendProgress?: (p: any) => void) {
    ...
    const dtype = info.id.includes("fp16") ? "fp16" : "q4";
    console.log(`📦 (Worker) Engine Request: ${info.name} (${category}) with ${dtype}...`);

    const commonOptions: any = {
      device: (this.isLowMemory || category === "music-gen" || category === "stt" || category === "tts") ? "wasm" : "webgpu",
      dtype: dtype,
      progress_callback: (p: any) => {
        if (sendProgress) sendProgress(p);
      }
    ...
    const tryLoad = async (options: any) => {
      ...
      } else if (info.id.includes("janus")) {
        this.processor = await AutoProcessor.from_pretrained(info.modelID);
        this.model = await AutoModelForImageTextToText.from_pretrained(info.modelID, options);
    ...
    try {
      ...
      if (isMemoryFault || isWebGPUError) {
        ...
        if (dtype === "fp16") {
          commonOptions.dtype = "q4"; // FP16 on WebAssembly is too slow, fallback to q4
        }
        try {
          await tryLoad(commonOptions);
          if (category !== "stt" && category !== "tts") {
  ...
  async runDirectorInference(input: string, modelId?: string, sendProgress?: (p: any) => void) {
    ...
    const systemPrompt = `You are Omnix Director. Your job is to route requests to the correct model.
    Available categories: @text, @image, @music, @vision, @coder.
    Respond with the category followed by the optimized prompt for that model.
    Example: "@image: a red dragon in the sky" or "@text: explain quantum physics".
    Current User Input: ${input}`;

    const output = await director(systemPrompt, {
      max_new_tokens: 128,
      temperature: 0.1,
      do_sample: false
    });
    ...
    const cleanOutput = text.includes("Current User Input:") ? text.split("Current User Input:")[1].split("\n").slice(1).join("\n").trim() : text.trim();
    
    console.log("🎬 (Worker) Director Output:", cleanOutput);
    
    let category = "text";
    let prompt = input;

    if (cleanOutput.startsWith("@image")) {
      category = "image-gen";
      prompt = cleanOutput.replace("@image:", "").replace("@image", "").trim();
    } else if (cleanOutput.startsWith("@music")) {
      category = "music-gen";
      prompt = cleanOutput.replace("@music:", "").replace("@music", "").trim();
    } else if (cleanOutput.startsWith("@vision")) {
      category = "vision";
      prompt = cleanOutput.replace("@vision:", "").replace("@vision", "").trim();
    } else if (cleanOutput.startsWith("@coder")) {
      category = "coder";
      prompt = cleanOutput.replace("@coder:", "").replace("@coder", "").trim();
    } else if (cleanOutput.startsWith("@text")) {
      category = "text";
      prompt = cleanOutput.replace("@text:", "").replace("@text", "").trim();
    }
  ...
  async runInference(category: string, input: any, options: any = {}, sendProgress?: (p: any) => void, onToken?: (token: string) => void) {
    ...
    try {
      ...
      if (this.currentModelId?.includes("janus")) {
        const isVision = category === "vision";
        const conversation = isVision 
          ? [{ role: "<|User|>", content: `<image_placeholder>\n${options.prompt || "Describe this image"}`, images: [input] }]
          : [{ role: "<|User|>", content: input }];
        
        const inputs = await this.processor(conversation, isVision ? {} : { chat_template: "text_to_image" });
        ...
        if (isVision) {
          ...
          const outputs = await this.model.generate_images({ ...inputs, min_new_tokens: num_image_tokens, max_new_tokens: num_image_tokens, do_sample: true });
